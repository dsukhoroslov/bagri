package com.bagri.tools.vvm.util;

import javax.swing.*;

public class Icons {
    //public static ImageIcon MAIN_ICON = ImageUtil.createImageIcon("com/bagri/tools/vvm/manager/icon.png");
    public static ImageIcon MAIN_ICON = ImageUtil.createImageIcon("com/bagri/tools/vvm/manager/bagri16x4.png");
    public static ImageIcon ADD_ICON = ImageUtil.createImageIcon("com/bagri/tools/vvm/manager/add.png");
    public static ImageIcon DELETE_ICON = ImageUtil.createImageIcon("com/bagri/tools/vvm/manager/delete.png");
    public static ImageIcon EDIT_ICON = ImageUtil.createImageIcon("com/bagri/tools/vvm/manager/edit.png");
    public static ImageIcon BAGRI_MANAGER_ICON = ImageUtil.createImageIcon("com/bagri/tools/vvm/manager/bagri16x32.png");
    public static ImageIcon USER_MANAGEMENT_ICON = ImageUtil.createImageIcon("com/bagri/tools/vvm/manager/userManagement.png");
    public static ImageIcon CLUSTER_MANAGEMENT_ICON = ImageUtil.createImageIcon("com/bagri/tools/vvm/manager/clusterManagement.png");
    public static ImageIcon SCHEMA_MANAGEMENT_ICON = ImageUtil.createImageIcon("com/bagri/tools/vvm/manager/schemaManagement.png");
    public static ImageIcon NODE_ICON = ImageUtil.createImageIcon("com/bagri/tools/vvm/manager/node.png");
    public static ImageIcon SCHEMA_ICON = ImageUtil.createImageIcon("com/bagri/tools/vvm/manager/schema.png");
    public static ImageIcon DOCUMENT_ICON = ImageUtil.createImageIcon("com/bagri/tools/vvm/manager/document.png");
    public static ImageIcon COLLECTION_ICON = ImageUtil.createImageIcon("com/bagri/tools/vvm/manager/collection.png");
    public static ImageIcon USER_ICON = ImageUtil.createImageIcon("com/bagri/tools/vvm/manager/user.png");
    private Icons() {}
}
