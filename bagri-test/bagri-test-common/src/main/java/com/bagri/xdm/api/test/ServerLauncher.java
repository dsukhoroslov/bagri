...
public class ServerLauncher {
	...
	public ServerLauncher(String profile, String[] props, String dir) {
		String java_opts = "-server -Xms1g -Xmx1g " +
				"-Dnode.name=first -Dxdm.config.path=src/main/resources " +
				"-Dxdm.config.context.file=spring/cache-system-context.xml " +
				"-Dxdm.config.properties.file=" + profile + ".properties " +
				"-Dxdm.config.filename=config.xml -Dxdm.log.level=info " +
	...
	public void startServer() {
		...
		try {
			...
			logger.info("startServer; XDM server has been started");
	...
	public void stopServer() {
		if (server != null) {
			...
			ith.interrupt();
			server.destroy();
			logger.info("stopServer; XDM server stopped with exitValue: {}", server.exitValue());
		} else {
