...
rem register schema
"%java_exec%" -server %java_opts% -cp "%app_home%\target\*;%app_home%\target\lib\*" com.bagri.common.manage.MBeanInvoker %admin_addr% %login% %password% com.bagri.xdm:type=Schema,name=default,kind=ModelManagement registerSchema ../../etc/samples/tpox/security.xsd java.lang.String
...
for /l %%x in (5, 1, 10) do (
 	echo %%x
rem 	"%java_exec%" -server %java_opts% -cp "%app_home%\target\*;%app_home%\target\lib\*" net.sf.tpox.workload.core.WorkloadDriver -w queries/securities-xqj.xml -u %%x 
