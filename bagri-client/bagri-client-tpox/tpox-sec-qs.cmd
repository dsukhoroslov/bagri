...
"%java_exec%" -server -showversion %java_opts% -cp "%app_home%\target\lib\*;%app_home%\target\*" net.sf.tpox.workload.core.WorkloadDriver -w properties/insSecurity-xqj.xml -tr 2604 -u 8 %*

rem get insert statistics
"%java_exec%" -server %java_opts% -cp "%app_home%\target\lib\*;%app_home%\target\*" com.bagri.client.tpox.StatisticsCollector localhost:3330 TPoX2 ./stats.txt InsertSecurities

rem perform queries loopig by user count
for /l %%x in (50, 10, 100) do (
	echo %%x
	"%java_exec%" -server %java_opts% -cp "%app_home%\target\lib\*;%app_home%\target\*" net.sf.tpox.workload.core.WorkloadDriver -w properties/queries-xqj.xml -u %%x %*
	"%java_exec%" -server %java_opts% -cp "%app_home%\target\lib\*;%app_home%\target\*" com.bagri.client.tpox.StatisticsCollector localhost:3330 TPoX2 ./stats.txt Users=%%x
)
