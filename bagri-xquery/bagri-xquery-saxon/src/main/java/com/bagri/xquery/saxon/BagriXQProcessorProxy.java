...
public class BagriXQProcessorProxy extends SaxonXQProcessor implements XQProcessor {
	...
	@Override
	public Object executeXCommand(String command, Map<QName, XQItemAccessor> bindings, XQStaticContext ctx) throws XQException {
		
		return executeXCommand(command, bindings, contextToMap(ctx));
	}

	@Override
	public Object executeXCommand(String command, Map<QName, XQItemAccessor> bindings, Map<String, Object> ctx) throws XQException {
		
    	logger.trace("executeXCommand.enter; command: {}", command);
    	XDMDocumentManagement dMgr = getXdmManager();
		return dMgr.executeXCommand(command, bindings, ctx);
	}
