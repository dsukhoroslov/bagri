...
#        ${JAVA_HOME}/bin/java ${JAVA_OPTS} com.bagri.xdm.cache.hazelcast.BagriCacheServer </dev/null >>"${stdoutfile}" 2>>"${stderrfile}" &
        java ${JAVA_OPTS} com.bagri.xdm.cache.hazelcast.BagriCacheServer </dev/null >>"${stdoutfile}" 2>>"${stderrfile}" &
        echo $! >"${pidfile}"
